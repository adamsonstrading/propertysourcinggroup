...
<head>
    ...
    <title>Edit How It Works Step - PSG Admin</title>
...
<body>
    ...
    <div class="main-content">
        <div class="admin-card">
            <h4 class="mb-4">Edit How It Works Step</h4>

            <form action="{{ route('admin.work-steps.update', $workStep->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    <label class="form-label fw-bold">Step Title</label>
                    <input type="text" name="title" class="form-control" required value="{{ $workStep->title }}">
                </div>
                <div class="mb-3">
                    <label class="form-label fw-bold">Description</label>
                    <textarea name="description" class="form-control" rows="3"
                        required>{{ $workStep->description }}</textarea>
                </div>
                <div class="mb-3">
                    <label class="form-label fw-bold">Sort Order</label>
                    <input type="number" name="sort_order" class="form-control" value="{{ $workStep->sort_order }}">
                </div>
                <div class="d-flex gap-2">
                    <button type="submit" class="btn btn-pink px-4">Update Step</button>
